...
pizzaGrande:
	MOVB R5, [R0]						;Coloca o valor de R0 bit a bit em R5 -> R0 tem o endereço na base de dados correspondente ao saldo acumulado
...
pizzaPequena:
	...
	MOVB R6, [R9]						;localização dos valores da pizza na DB
	ADD R6, R3
	MOVB [R9], R6
	JMP pedidoSucesso					;Salta para a rotina pedidoSucesso
...
menubebidas:
    ...
    PUSH R4                             ;-               								 - 
    PUSH R5 							;-------------------------------------------------
	MOV R0, R10							;Coloca o endereço de R10 em R1 -> localização do valor acumulado  na DB
	MOV R9, R10							
	ADD R9, 7							;Localização do preço acumulado da compra ATUAL
...
compraagua:
	MOV R2, 1							;Preço da agua
	MOVB R5, [R0]						;Coloca o valor de R0 bit a bit em R5 -> localização do historico acumulado
	ADD R5, R2							;Adiciona o endereço de R2 no endereço de R5
...
verifica_Desconto:
	...
	MOV R3, 50
	SUB R9, 2
	MOVB R8, [R10]				;localização do valor total acumulado da db
	MOVB R6, [R9]

verifica_cinquenta:
	CMP R8, R3					;verifica se o valor é superior a 50euros a verificar se irá existir desconto
	JLE Converte_to_Char

ciclo_Verifica_pizzas_grandes:
	MOV R4, R6
	MOD R4, R0					;resto de divisão inteira do valor das pizzas por 9 -> se o resto for igual a 0 quer dizer que só existem pizzas grandes na ordem caso contrário existem pizzas pequenas
	CMP R4, R2
	JEQ desconta_quatro_cinco
	JNE desconta_dois_cinco

desconta_quatro_cinco:
	SUB R6, 5					;Desconto de 4,50 -> é subraido 5 euros aqui pq corresponde ao valor das unidades, posteriormente serão adicionados os 50 centimos 
	ADD R9, 2
	MOVB [R9], R6
